...
Run commands from the repository root after loading ESP-IDF, for example `source $IDF_PATH/export.sh`.
...
Git history is currently minimal (`init`, `// wip`), so prefer clearer future commits: imperative intent line, concise rationale, and relevant Lore trailers such as `Tested:` and `Not-tested:`. Pull requests should describe the firmware behavior changed, list build/test results, link issues, and include screenshots or serial logs for UI/display changes.
