...
- 사용자가 실기기에서 키보드 입력이 정상 동작함을 확인함. 반복 연결/해제 10회 및 실제 키 repeat 30초 스트레스는 개별 체크 항목으로 유지한다.
- monitor 로그에서 HID keyboard VID/PID 연결, LEFT_SHIFT/LEFT_ALT press/release, ARROW_LEFT/ARROW_RIGHT press/release 수신 확인.
- 2026-05-30 monitor 세션에서 키보드 disconnect 6회 / connect 6회까지 앱 panic 없이 `Input status connected=0/1` 처리 확인. 10회 반복 조건은 아직 미충족.
...
- [ ] `dkssud` → `안녕`
- [ ] `rhk` → `과`
- [ ] `ho` → `ㅙ` 또는 정책에 맞는 조합 결과
- [ ] `r` + Backspace → 빈 조합
- [ ] `gks` + Backspace → `하`
- [ ] `rk` + `s` + `k` → `가나` (종성 분리 규칙)
- [ ] `Ctrl+Space`로 EN/KO 전환
- [ ] EN 모드에서 기존 ASCII 입력이 Phase 3 동작과 동일
- [ ] 조합 중 방향키 입력 시 commit 후 커서 이동
- [ ] UTF-8 한글 삽입/삭제 후 문자열이 깨지지 않음
- [ ] undo 1회로 직전 commit 문자열이 정상 제거됨
