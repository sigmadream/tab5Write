...
작성일: 2026-05-26
기반 문서: TODO.md (포팅 계획서)
대상: ESP32-P4 / M5Stack Tab5 / ESP-IDF 6.0.1
목적: TODO.md의 각 Phase를 실제 구현 가능한 세부 작업 단위로 분해하고, 작업 간 의존 관계/위험 요소/검증 기준을 명시한다.
...
- [ ] `IDF_TARGET=esp32p4` 설정된 새 CMake 프로젝트 골격 생성
- [ ] 루트 `CMakeLists.txt` 작성 (cmake_minimum_required, project, component 경로)
- [ ] `main/CMakeLists.txt` 작성 (SRCS, INCLUDE_DIRS)
- [ ] `sdkconfig.defaults` 작성
  ...
  - 로그 레벨 INFO
- [ ] `partitions.csv` 작성 (nvs / phy_init / factory / storage 구성)

### 1-2. 최소 app_main 구현

- [ ] `main/app_main.cpp` 작성
  - `nvs_flash_init()` 호출
...
- [ ] `main/app_build_info.h` 작성 (버전 매크로, 컴파일 타임스탬프)
...
- [ ] 태스크 생성 뼈대 (실제 핸들러는 Phase 2~3에서 구현)
  - ui_task placeholder (5초마다 heartbeat 로그)
  - storage_task placeholder (큐 대기만)

### 1-4. 로깅 규약 확립

- [ ] 통합 로그 태그 접두사 `SCRIBE_` 적용
  - `SCRIBE_MAIN`, `SCRIBE_UI`, `SCRIBE_INPUT`, `SCRIBE_STORAGE` 등
- [ ] `ESP_LOGI` / `ESP_LOGW` / `ESP_LOGE` 사용 규칙 문서화

### 검증 체크리스트

- [ ] `idf.py set-target esp32p4` 성공
- [ ] `idf.py fullclean build` 0 error, 0 warning (또는 IDF 내부 warning만)
- [ ] `idf.py flash monitor` 후 panic/watchdog 없이 boot 완료
- [ ] 모니터 로그에 IDF version, target, heap, PSRAM, partition 정보 출력 확인
- [ ] 10분 idle 후에도 watchdog 발생 없음

### 위험 요소

- IDF 6.0.1의 sdkconfig 키 이름이 5.x와 다를 수 있음 → `idf.py menuconfig`로 실제 키 확인 필요
- PSRAM 초기화 실패 시 heap 크기가 극도로 작게 보임 → PSRAM config 재확인
