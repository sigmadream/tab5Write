...
#### 1. 개발용 LVGL 스냅샷/캡처 경로

- [x] LVGL snapshot 기능 활성화
  - `CONFIG_LV_USE_SNAPSHOT=y`
- [x] 활성 화면을 USB Serial/JTAG로 덤프하는 명령 경로 추가
  - serial command task에서 `snapshot` 또는 `snap` 입력 시 UI 이벤트 발행
  - UI task가 현재 writing screen을 갱신한 뒤 snapshot dump 실행
- [x] 개발 과정 화면 기록용 PC 저장 흐름 확보
  - 기기: LVGL 내부 snapshot 생성
  - 전송: USB/Serial 로그 스트림
  - PC: 캡처 산출물 저장
- [x] 검증
  - `idf.py build` 통과
  - flash/monitor에서 serial command task 시작 및 snapshot command enqueue 경로 확인

#### 2. Tab5 Keyboard 컴포넌트 추가

- [x] `components/tab5_keyboard/` 컴포넌트 추가
  - Tab5 Keyboard I2C HID 이벤트 polling
  - HID mode 설정
  - keyboard brightness / RGB binding 기본 설정
  - HID event → `Tab5KeyboardHidEvent` callback 제공
- [x] `main/app_input.cpp`에 Tab5 Keyboard 입력 소스 통합
  - 기존 USB HID keyboard와 동일한 `KeyEvent` 경로로 변환
  - key repeat 상태를 USB/TAB5 입력 소스별로 분리
  - modifier 상태를 USB + TAB5 합성 상태로 처리
- [x] Tab5 Keyboard 미장착 허용
  - 미검출은 fatal error가 아니라 optional device absence로 처리
  - 최초 미검출은 INFO 1회만 출력
  - 초기 3회는 1초 간격, 이후 30초 간격으로 백그라운드 재시도
  - monitor task 생성 실패 시에도 앱 전체는 계속 동작
- [x] 검증
  - `idf.py build` 통과
  - `test_editor_core`, `test_keybinding`, `test_ime` 직접 실행 통과

#### 3. LVGL 로그 레벨 조정

- [x] LVGL info 로그 과다 출력 억제
  - `CONFIG_LV_USE_LOG=y` 유지
  - `CONFIG_LV_LOG_LEVEL_WARN=y` 적용
  - `CONFIG_LV_LOG_PRINTF=y` 유지
- [x] 검증
  - `idf.py build` 통과
  - monitor에서 LVGL `[Info] lv_obj_create`, `indev_proc_press` 류 로그가 사라지고 앱 로그는 유지됨을 확인

### 보류

#### 한글 폰트 렌더링

- [x] TinyTTF 기반 SD 카드 TTF 로더 실험
  - `/sdcard/fonts/tabwrite-kr.ttf`
  - `/sdcard/fonts/NotoSansKR-Regular.ttf`
  - `/sdcard/fonts/Pretendard-Regular.ttf`
  - `/sdcard/fonts/NanumGothic.ttf`
  - 위 후보를 TinyTTF로 로드하는 `app_font` provider를 구현했음
- [x] TinyTTF 실기 리스크 확인
  - `ttf_get_glyph_dsc_cb: cache not allocated` 반복 발생
  - cache 축소, kerning off, no-cache 직접 렌더링까지 시도했으나 렌더링 속도가 느려 실사용 부적합
- [x] 최종 경로를 사전 변환 LVGL C font로 전환
  - `x12y12pxMaruMinyaHangul_36.c`를 editor 기본 폰트로 고정
  - SD 카드 mount / TTF 탐색 / TinyTTF 런타임 rasterization 제거
  - `CONFIG_LV_USE_TINY_TTF` 비활성화
  - `CONFIG_LV_USE_FS_STDIO` 비활성화
  - footer 상태 문구: `Font built-in C`
- [x] 검증
  - `idf.py build` 통과
  - `TABWRITE.bin` 크기: `0x1138c0` bytes, app partition 64% free
  - `test_editor_core`, `test_keybinding`, `test_ime` 직접 실행 통과
...
### P0. 사전 변환 한글 폰트 범위 확장

- [ ] 현재 `x12y12pxMaruMinyaHangul_36.c`에 없는 한글 glyph 목록 수집
  - 실기에서 깨지는 글자 또는 빈 glyph가 나오는 문자열을 기록
  - IME 조합 결과 중 누락되는 완성형 한글 우선 수집
- [ ] 폰트 생성 스크립트/절차를 `tools/` 또는 문서로 고정
  - 입력 TTF 파일
  - 포함할 Unicode range/symbol list
  - LVGL 변환 옵션 (`--bpp`, `--size`, `--format lvgl` 등)
- [ ] 더 넓은 한글 subset 또는 완성형 한글 전체 포함 여부 판단
  - 전체 포함 시 flash 크기 증가 리스크 있음
  - MVP는 실제 입력 빈도 기반 subset 확장이 더 안전
- [ ] 생성된 C font로 `idf.py build` 및 실기 렌더링 확인

### P0. 입력 안정성 실기 확인

- [ ] Tab5 Keyboard 장착/미장착 각각 cold boot 확인
- [ ] Tab5 Keyboard 연결 후 키 입력/반복 입력/수식어 조합 확인
- [ ] USB Keyboard와 Tab5 Keyboard가 동시에 존재할 때 modifier 합성 상태 확인
- [ ] Tab5 Keyboard 분리 또는 I2C 오류 후 앱이 계속 동작하는지 확인

### P0. writing screen 성능 측정

- [ ] key-to-render latency 로그 기준으로 긴 한글 문장 입력 시 지연 측정
- [ ] C font 전환 후 커서 이동/줄바꿈/스크롤 성능 확인
- [ ] 10분 이상 연속 입력에서 watchdog, heap corruption, display artifact 확인

### P1. 폰트/텍스트 렌더링 품질 개선

- [ ] editor font 크기 36px가 실제 writing screen 밀도에 적절한지 재평가
- [ ] line height / baseline / line gap 조정
- [ ] fallback glyph가 필요한 경우 fallback chain 정책 재정의
- [ ] UI 라벨에도 한국어가 필요해지면 별도 작은 크기 C font 추가 검토

### P1. 개발 캡처 흐름 정리

- [ ] Serial snapshot PC 저장 스크립트 사용법 문서화
- [ ] 캡처 파일 naming 규칙 정리
- [ ] 렌더링 회귀 비교용 sample text/screenshot 세트 추가

### P1. 설정/문서 정리

- [ ] `sdkconfig`는 커밋하지 않고 `sdkconfig.defaults`만 intentional default로 관리
- [ ] `LESSONS_LEARNED.md`에 TinyTTF 실기 리스크 기록
  - cache allocation failure
  - no-cache 렌더링 속도 문제
  - ESP32-P4/Tab5에서는 MVP 경로로 C font 권장
- [ ] `README.md`에 현재 입력 장치 지원 범위 업데이트
  - USB Keyboard
  - optional Tab5 Keyboard
  - serial snapshot command