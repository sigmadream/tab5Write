...
- [x] 통합 로그 태그 접두사 `TABWRITE_` 적용
  - `TABWRITE_MAIN`, `TABWRITE_UI`, `TABWRITE_INPUT`, `TABWRITE_STORAGE` 등
...
- [x] `idf_component.yml`에 BSP 의존성 추가
  - `m5stack_tab5`를 `ref/esp-bsp/bsp/m5stack_tab5` local override로 고정
...
- [x] BSP 초기화 코드 작성
  ...
  - ESP-BSP 초기화 순서 사용
- [x] 백라이트 제어 구현
  - GPIO 22 (LEDA) on/off 제어로 Phase 2 범위에 맞게 단순화
...
components/
  ...
  tabwrite_input/           → USB keyboard, key normalization, keybindings
