...
Use C++ source files (`.cpp`) with lightweight headers (`.h`) in `main/`. Follow the existing two-space indentation and keep braces on the same line. Prefer `snake_case` for functions and variables (`app_queue_init`, `display_set_backlight`) and `UPPER_SNAKE_CASE` for macros or log tags. Keep module prefixes (`app_`, `display_`) to make ownership obvious. Use ESP-IDF error handling macros such as `ESP_ERROR_CHECK` and LVGL APIs consistently. When adding hardware, display, input, or board-support code, use `ref/` only as read-only reference material and reproduce the needed pattern inside owned project code.

## Reference Directory Boundary
`ref/` is reference material only. Do not directly use, depend on, vendor, include, compile, patch, or runtime-load files from `ref/` as part of the project implementation. In particular, do not add `override_path`, CMake source/include paths, component dependencies, scripts, or generated outputs that point into `ref/`. If a referenced implementation is needed, copy or adapt the minimal relevant idea into project-owned files and keep the dependency path outside `ref/`.
